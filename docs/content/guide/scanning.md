...
|----------|----------------|
| `headers` | Security headers (HSTS, CSP incl. report-only-only, XFO, Permissions-Policy, …), cleartext Basic auth, mixed content, cacheable API responses, MIME-type confusion (an HTML body under a sniffable type with no `nosniff`, JSON served as `text/html`), JWT weaknesses (`alg:none`, non-standard alg, no `exp`), cross-origin subresources without `integrity` |
...
- **Out-of-band rules** (blind SSRF) ship **enabled but inert**: they can only send once the project has a registered OAST listener to mint a payload from. That is a capability, not a toggle, so the row is badged `needs OAST` and its request cost stays out of the estimate until a listener exists.
