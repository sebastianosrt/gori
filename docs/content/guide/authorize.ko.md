...
- 그 슬롯의 **헤더 오버레이** 가 최종 와이어 바이트에 적용됩니다 — `$NAME` 치환 **이후**, 헤더 줄에만. 그래서 `Content-Length`는 움직이지 않고 본문은 바이트 그대로입니다.
...
여기에는 쿠키 항아리도, 자동 로그인 매크로도 없습니다. 슬롯은 당신이 쓴 헤더와 gori가 관찰한 값을 들고 있고, `--bind-from`은 당신이 지목한 플로우 하나를 재생해 그 값을 채웁니다.
