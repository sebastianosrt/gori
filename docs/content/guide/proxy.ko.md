...
붙잡힌 메시지를 읽는 데는 표시도 편집기도 필요 없습니다. 미리보기는 줄을 접어 보여주므로 옆으로 벗어나는 내용이 없고, 휠로 스크롤하거나 `↵` / `e`로 편집기를 열면 `PgUp` / `PgDn`이 본문을 넘깁니다. `PgUp` / `PgDn` / `Home` / `End`는 다른 목록 탭과 마찬가지로 **큐**를 넘깁니다. `↑` / `↓`가 걷는 그 목록입니다.

각 행에는 그 메시지가 얼마나 기다렸는지가 함께 표시됩니다. 붙잡힌 메시지는 결정을 기다리며 멈춰 있는 실제 클라이언트이므로, 한꺼번에 몰린 홀드가 단순한 목록이 아니라 큐로 읽힙니다. 위쪽 캐치 바에는 실제로 걸려 있는 조건이 보입니다. MCP나 `gori run intercept filter`로 에이전트가 설정한 조건도 마찬가지입니다. 그리고 게이트가 캐치를 켜둔 메시지를 붙잡지 못했을 때 — 버퍼 한도를 넘긴 바디는 잘라내지 않고 그대로 흘려보냅니다 — 오지 않을 행을 기다리게 두는 대신 알림 센터에 그 사실을 남깁니다.
...
본문 아래 정보 띠에 `sent by gori — repeater (tui) #4`처럼 풀어 적어, 어느 표면에서 왔고 어느
세션이 만든 것인지까지 되짚을 수 있게 합니다. 그 띠에는 상태 코드, 프로토콜, 요청·응답 크기,
응답 시간, content type, 캡처 시각도 함께 실리며 어느 패널에서든 같은 자리에 있습니다. 패널
자체는 와이어의 바이트 그대로입니다.
...
쿼리는 *어떤 플로우가 걸리는가*에 답합니다. **컬럼(column)**은 나머지 절반, 즉 *각 행에서 X의 값은 무엇인가*에 답합니다. `Space` 다음 `C`를 누르면 컬럼을 추가할 수 있습니다(`X-Request-Id`, JWT의 `sub`, rate-limit 헤더, JSON 본문 속 필드). 그러면 그 값이 목록의 모든 플로우 옆에 그려집니다.
...
플로우 하나 위에서 `Space` `Y`를(History에서, 상세 뷰의 REQUEST 패널에서, Repeater 탭에서) 누르면 **Copy as…**가 열립니다. 같은 요청을 다른 도구가 읽는 모양으로 내줍니다.
...
- **Enable/disable everywhere**(`Space → X`, 직접 키 없음 — `Space → X`는 clear가 있는 탭들의 전체 삭제 코드입니다)는 전역 기본값 자체를 뒤집습니다. 오버라이드하지 않은 모든 프로젝트가 이 값을 따릅니다.
...
나머지 네 동작은 이미 존재하는 메시지를 재작성합니다. **Short circuit**은 대신 답합니다. 요청이 매칭되면 gori가 직접 작성한 응답을 돌려주고, 원본에는 접속하지 않습니다. Replace 규칙으로는 구조적으로 불가능한 경우를 덮습니다. 엔드포인트가 404·500을 내거나, 원본이 꺼져 있거나 인증 벽 뒤에 있거나, 본문을 유도하는 게 아니라 만들어 내야 하는 경우입니다.
...
규칙은 Match & Replace 규칙과 똑같이 이 프로젝트 또는 모든 프로젝트가 읽는 **전역 라이브러리** 중 한쪽에 삽니다. `s`로 둘 사이를 옮기고, `x`로 여기서 켜고 끄고, `Space → X`로 전역 규칙의 기본값을 모든 곳에서 뒤집습니다. 라이브러리와 의견이 다른 프로젝트는 그 *불일치만* 저장하며, 다시 의견이 같아지는 순간 불일치는 삭제됩니다. 껐다 켠 규칙이 오늘의 답에 고정되는 대신 라이브러리를 다시 따라가게 되는 이유입니다.

| 동작 | 키 |
|--------|-----|
| 규칙 추가 / 편집 | `a` / `Enter` 또는 `e` |
| 여기서 켜기·끄기 | `x` |
| 전역 규칙의 기본값을 모든 곳에서 뒤집기 | `Space → X` |
| 프로젝트 ⇄ 전역 이동 | `s` |
...
**HAR은 WebSocket 메시지를 양방향으로 실어 나릅니다.** 캡처된 소켓은 실제 `101` 핸드셰이크로 익스포트되고, 메시지 로그는 Chrome DevTools의 `_webSocketMessages` 필드에 나란히 담깁니다. 가짜 HTTP 교환으로 꾸며내지 않습니다. 그 HAR을 임포트하면 메시지가 플로우에 복원되므로, 전달 과정을 거친 뒤에도 WebSocket 창, `gori run show`, 그리고 재익스포트에서 그대로 보입니다. 각 메시지는 방향, opcode(코드와 사유가 담긴 `CLOSE`, `PING`, `PONG` 같은 제어 프레임 포함), 바이트(유효한 UTF-8이 아니면 base64로 담기므로 바이너리 프레임이나 의도적으로 잘못된 UTF-8 프레임도 정확히 왕복합니다), 밀리초 단위 타임스탬프를 유지합니다. gori가 남긴 `[gori] …` 알림 행도 함께 이동합니다. 스트림에서의 위치가 그 행이 가리키는 프레임을 지목하기 때문입니다. HAR이 담지 못하는 것은 두 가지입니다: 프레임별 형태(`FIN`/`RSV`/마스크 키/프래그먼트 수)는 `--format json` 또는 `raw`를 쓰고, 전송 기록이 *비어 있는* 소켓은 여전히 건너뛰고 개수를 보고합니다. 핸드셰이크만으로는 교환이 아니기 때문입니다.
